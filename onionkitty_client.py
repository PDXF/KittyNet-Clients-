...
class KittyNet(QWidget):
    ...
    def init_ui(self):
        self.setWindowTitle("KittyNet - Tor Edition")
    ...
    def connect_to_server(self):
        try:
            ...
            if "200 OK" in response:
                self.connected = True
                self.user_list.item(3).setText("Server Status: ✅")
    ...
    def receive_messages(self):
        while self.connected:
            try:
                ...
                if message:
                    self.display_message(f"[Server]: {message}", False)
            except socket.error:
                break

    def display_message(self, message, is_sender):
        if is_sender:
            self.chat_display.append(f"<p style='color: #9B4F96;'> {message}</p>")
        else:
            self.chat_display.append(f"<p style='color: #9B4F96;'> {message}</p>")

if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = KittyNet(SERVER_HOST, SERVER_PORT)
    window.show()
    sys.exit(app.exec_())
